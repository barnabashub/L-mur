...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: spacing.l, gap: spacing.m }}>
      ...
      <Card style={{ gap: 6 }}>
        <Text style={{ fontSize: 20, fontWeight: '800', color: t.text }}>{me.name}</Text>
        <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          <Text style={{ color: t.muted, fontSize: 13 }}>{me.email}</Text>
        ...
        <View style={{ flexDirection: 'row', gap: spacing.l, marginTop: spacing.m }}>
          {[
            ...
            <View key={String(label)} style={{ alignItems: 'center', flex: 1 }}>
              <Text style={{ fontSize: 18, fontWeight: '800', color: t.primary }}>{n}</Text>
              <Text style={{ fontSize: 11, color: t.muted, textAlign: 'center' }}>{label}</Text>
            </View>
          ))}
        </View>
      </Card>
      ...
      <Card style={{ gap: spacing.s }}>
        <Text style={{ fontWeight: '700', color: t.text }}>Párom 💑</Text>
        {me.partner ? (
