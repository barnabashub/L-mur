    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-mute">
          ...
          {!partner && (
            <>
              {' '}
              <Link href="/par" className="font-medium text-brand hover:underline">Pár összekapcsolása →</Link>
            </>
          )}
        </p>
      </div>
      ...
      {entries.length === 0 ? (
        <p className="card p-10 text-center text-mute">
          A naplótok még üres. Pipáljatok ki egy <Link href="/otletek" className="text-brand hover:underline">randiötletet</Link>,
          ...
          {entries.map((entry) => {
            ...
            return (
              <li key={`${entry.kind}-${entry.item.id}`} className="relative">
                <span className="absolute -left-[31px] top-1 flex h-4 w-4 items-center justify-center rounded-full bg-brand text-[8px] text-white">
                    ...
                    {entry.kind === 'completion' ? (
                      <>
                        <h3 className="font-semibold">
                          <Link href={`/otletek/${entry.item.idea.id}`} className="hover:text-brand">
                            {entry.item.idea.title}
                          </Link>
                        </h3>
                        {entry.item.publicText && <p className="text-sm text-ink/90">{entry.item.publicText}</p>}
                        {entry.item.privateText && (
                          <p className="rounded-lg bg-brand-soft/60 px-3 py-2 text-sm text-ink/90">
                            🔒 {entry.item.privateText}
                          </p>
                        )}
                        {!entry.item.imagePublic && entry.item.imagePath && (
                          <p className="text-xs text-faint">A fotó privát — csak itt, a naplóban látszik.</p>
                        )}
                        {isOwn && (
                      ...
                      <>
                        ...
                        {entry.item.text && <p className="text-sm text-ink/90">{entry.item.text}</p>}
                        {isOwn && (
