  ...
  return (
    <div className="space-y-10">
      ...
      {idea.status !== 'APPROVED' && (
        <p className="rounded-blob border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:text-amber-200 dark:border-amber-400/25 dark:bg-amber-400/10 dark:text-amber-300">
          {idea.status === 'PENDING'
      ...
      <section className="card overflow-hidden">
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-ink">{idea.title}</h1>
          ...
          <p className="whitespace-pre-line leading-relaxed text-ink/90">{idea.description}</p>
          <p className="border-t border-edge pt-4 text-xs text-faint">
            Feltöltötte: <span className="font-medium text-mute">{idea.submitter.name}</span>
        ...
        <section className="panel-amber p-6">
          ...
          {user ? (
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <p className="inline-block rounded-lg border border-dashed border-amber-400 bg-card px-4 py-2 font-mono text-lg font-bold tracking-widest text-amber-800 dark:text-amber-200">
          ...
          <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">Mutasd fel a kódot a helyszínen. Részletek: <Link href="/partnerek" className="underline">partnereink</Link>.</p>
        </section>
      )}
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="card p-6">
            ...
            <form action={completeIdea} className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm text-ink/90">
                <input type="checkbox" name="imagePublic" className="accent-violet-500" />
          ...
          <div className="space-y-6">
                    ...
                    <li key={c.id} className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        ...
                        {(c.publicText || c.privateText) && (
                          <p className="line-clamp-1 text-mute">{c.publicText ?? c.privateText}</p>
                        )}
                      </div>
                      <Link href="/naplo" className="ml-auto text-xs font-medium text-brand hover:underline">
                        Napló →
                      </Link>
                    </li>
                  ))}
            ...
            <div className="card p-6">
              ...
              {myLists.length === 0 ? (
                <p className="text-sm text-mute">
                  ...
                  <Link href="/bakancslistak" className="font-medium text-brand hover:underline">
                    hozz létre egyet
                  </Link>
                  .
                </p>
              ) : listsWithout.length === 0 ? (
                <p className="text-sm text-mute">Ez az ötlet már minden listádon rajta van. 💪</p>
              ) : (
            ...
            <details className="card p-6">
              <summary className="cursor-pointer font-semibold text-ink/90">
              ...
              return (
                <figure key={c.id} className="card overflow-hidden">
                  ...
                  <figcaption className="space-y-1 p-4 text-sm">
                    {c.publicText && <p className="text-ink/90">„{c.publicText}"</p>}
                    {showPrivate && c.privateText && (
                      <p className="text-mute">🔒 {c.privateText}</p>
                    )}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-3">
            {idea.reviews.length === 0 ? (
              <p className="card p-6 text-sm text-mute">Még senki sem értékelte — legyetek ti az elsők!</p>
            ) : (
              idea.reviews.map((r) => (
                <div key={r.id} className="card p-4">
                  <div className="flex items-center justify-between">
                    <Stars value={r.stars} />
                    <span className="text-xs text-faint">{r.user.name} · {formatDate(r.createdAt)}</span>
                  </div>
                  {r.text && <p className="mt-2 text-sm text-ink/90">{r.text}</p>}
                </div>
              ))
            )}
          </div>
